# `security-governance-service` — Kubernetes binding

This service runs as part of the umbrella Helm chart at
`infra/k8s/helm/open-foundry/`. The only state owned by this bounded
context is the **security-governance-service** Postgres schema (migrations under
...
| Read/write DNS | `security-governance-pg-rw.openfoundry.svc.cluster.local:5432`             |
| Read-only DNS  | `security-governance-pg-ro.openfoundry.svc.cluster.local:5432`             |
| Database       | `security_governance`                                                       |
| App secret     | `security-governance-pg-app` (CNPG-managed; carries `username`, `password`, `uri`) |
| Backup secret  | `security-governance-pg-backup` (S3 credentials for barman-cloud)          |
...
`services.security-governance-service.envSecrets.DATABASE_URL`
