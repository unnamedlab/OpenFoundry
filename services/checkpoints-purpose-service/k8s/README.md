# `checkpoints-purpose-service` — Kubernetes binding

This service runs as part of the umbrella Helm chart at
`infra/k8s/helm/open-foundry/`. The only state owned by this bounded
context is the **checkpoints-purpose-service** Postgres schema (migrations under
...
| Field          | Value                                                              |
|----------------|--------------------------------------------------------------------|
| Manifest       | [`infra/k8s/platform/manifests/cnpg/clusters/checkpoints-purpose-pg.yaml`](../../../infra/k8s/platform/manifests/cnpg/clusters/checkpoints-purpose-pg.yaml) |
| `kind`         | `Cluster` (`postgresql.cnpg.io/v1`, CloudNativePG / cloudnative-pg)|
| Cluster name   | `checkpoints-purpose-pg`                                                   |
| Namespace      | `openfoundry`                                                      |
| Read/write DNS | `checkpoints-purpose-pg-rw.openfoundry.svc.cluster.local:5432`             |
| Read-only DNS  | `checkpoints-purpose-pg-ro.openfoundry.svc.cluster.local:5432`             |
| Database       | `checkpoints_purpose`                                                       |
| App secret     | `checkpoints-purpose-pg-app` (CNPG-managed; carries `username`, `password`, `uri`) |
| Backup secret  | `checkpoints-purpose-pg-backup` (S3 credentials for barman-cloud)          |
...
`services.checkpoints-purpose-service.envSecrets.DATABASE_URL`
