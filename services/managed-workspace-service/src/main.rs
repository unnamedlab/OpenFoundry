...
#[tokio::main]
async fn main() {
    observability::init_tracing("managed-workspace-service");
    ...
    let public = Router::new().route(
        "/health",
        get(|| async { axum::Json(HealthStatus::ok("managed-workspace-service")) }),
    );
    ...
    let protected = Router::new()
        ...
        .route(
            "/api/v1/managed-workspaces/workspaces/{id}",
            get(handlers::get_item),
        )
        .route(
            "/api/v1/managed-workspaces/workspaces/{id}/secondary",
    ...
    tracing::info!("starting managed-workspace-service on {addr}");
