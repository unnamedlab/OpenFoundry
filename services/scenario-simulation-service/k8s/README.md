# `scenario-simulation-service` — Kubernetes binding

This service runs as part of the umbrella Helm chart at
`infra/k8s/helm/open-foundry/`. The only state owned by this bounded
context is the **scenario-simulation-service** Postgres schema (migrations under
...
| Field          | Value                                                              |
|----------------|--------------------------------------------------------------------|
| Manifest       | [`infra/k8s/platform/manifests/cnpg/clusters/scenario-simulation-pg.yaml`](../../../infra/k8s/platform/manifests/cnpg/clusters/scenario-simulation-pg.yaml) |
| `kind`         | `Cluster` (`postgresql.cnpg.io/v1`, CloudNativePG / cloudnative-pg)|
| Cluster name   | `scenario-simulation-pg`                                                   |
| Namespace      | `openfoundry`                                                      |
| Read/write DNS | `scenario-simulation-pg-rw.openfoundry.svc.cluster.local:5432`             |
| Read-only DNS  | `scenario-simulation-pg-ro.openfoundry.svc.cluster.local:5432`             |
| Database       | `scenario_simulation`                                                       |
| App secret     | `scenario-simulation-pg-app` (CNPG-managed; carries `username`, `password`, `uri`) |
| Backup secret  | `scenario-simulation-pg-backup` (S3 credentials for barman-cloud)          |
...
`services.scenario-simulation-service.envSecrets.DATABASE_URL`
