...
| Field          | Value                                                              |
|----------------|--------------------------------------------------------------------|
| Manifest       | [`infra/k8s/platform/manifests/cnpg/clusters/oauth-integration-pg.yaml`](../../../infra/k8s/platform/manifests/cnpg/clusters/oauth-integration-pg.yaml) |
| `kind`         | `Cluster` (`postgresql.cnpg.io/v1`, CloudNativePG / cloudnative-pg)|
| Cluster name   | `oauth-integration-pg`                                                   |
| Namespace      | `openfoundry`                                                      |
| Read/write DNS | `oauth-integration-pg-rw.openfoundry.svc.cluster.local:5432`             |
| Read-only DNS  | `oauth-integration-pg-ro.openfoundry.svc.cluster.local:5432`             |
| Database       | `oauth_integration`                                                       |
| App secret     | `oauth-integration-pg-app` (CNPG-managed; carries `username`, `password`, `uri`) |
| Backup secret  | `oauth-integration-pg-backup` (S3 credentials for barman-cloud)          |
...
`services.oauth-integration-service.envSecrets.DATABASE_URL`
