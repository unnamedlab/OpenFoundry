...
| 1.5 | Tier-2 libs (cassandra-kernel, authz-cedar, saga, state-machine, scheduling, search/storage/vector abstractions, geospatial, media-scanner, ontology-kernel, pipeline-expression, plugin-sdk, analytical-logic, ai-kernel, ml-kernel) | 🟡 in progress — `libs/authz-cedar-go` ✅ feature-complete; `libs/ai-kernel-go` ✅ models (7/7) + domain/{llm cache+gateway+guardrails+provider+runtime placeholder, agents memory+planner, rag full, evaluation, copilot} + handlers (5/5: tools/prompts/knowledge/agents/chat with chat completion+copilot+benchmark stubbed at 501 pending runtime); `libs/ml-kernel-go` ✅ models (10/10) + domain/{drift, predictions, training/hyperparameter} + handlers (7/7: overview/predictions/training/features/deployments/models/experiments with runs + asset-lineage + create_model_version + create_training_job stubbed at 501 pending interop). Pending in ai-kernel-go: domain/llm/runtime full (581 LOC), domain/agents/executor (1307 LOC). Pending in ml-kernel-go: domain/{interop 769 LOC, training/runner 418 LOC, training/execute_training 191 LOC}. Other tier-2 libs migrate alongside first consuming service. |
...
| 6 | ML/AI/apps & retire Rust | 🟡 in progress — all 8 architecture+slice / foundation / substrate service ports done: `code-repository-review-service` ✅, `media-transform-runtime-service` ✅, `entity-resolution-service` ✅, `ontology-exploratory-analysis-service` ✅, `reindex-coordinator-service` ✅, `lineage-service` ✅, `workflow-automation-service` ✅, `federation-product-exchange-service` ✅. **`libs/ai-kernel-go`** ✅ full handler surface + every pure-logic domain except llm/runtime + agents/executor (see Phase 1.5). **`libs/ml-kernel-go`** ✅ full handler surface + drift/predictions/hyperparameter domain; interop + training/runner pending (see Phase 1.5). 16 substrate-only or full-foundation shells (llm-catalog, ai-evaluation, retrieval-context, agent-runtime, model-catalog, model-deployment, solution-design, application-composition, code-repo-review, media-transform-runtime, entity-resolution, ontology-exploratory-analysis, reindex-coordinator, lineage, workflow-automation, federation-product-exchange) all bind the kernel handlers today. Remaining: domain/llm/runtime + domain/agents/executor + domain/interop ports unblock the 501 stubs in chat completion / agent execution / training execution / model-version registration / asset lineage / runs CRUD; pyo3 sidecars STOP-and-ask. |
