# `notification-alerting-service` — Kubernetes binding

This service runs as part of the umbrella Helm chart at
`infra/k8s/helm/open-foundry/`. The only state owned by this bounded
context is the **notification-alerting-service** Postgres schema (migrations under
...
| Field          | Value                                                              |
|----------------|--------------------------------------------------------------------|
| Manifest       | [`infra/k8s/platform/manifests/cnpg/clusters/notification-alerting-pg.yaml`](../../../infra/k8s/platform/manifests/cnpg/clusters/notification-alerting-pg.yaml) |
| `kind`         | `Cluster` (`postgresql.cnpg.io/v1`, CloudNativePG / cloudnative-pg)|
| Cluster name   | `notification-alerting-pg`                                                   |
| Namespace      | `openfoundry`                                                      |
| Read/write DNS | `notification-alerting-pg-rw.openfoundry.svc.cluster.local:5432`             |
| Read-only DNS  | `notification-alerting-pg-ro.openfoundry.svc.cluster.local:5432`             |
| Database       | `notification_alerting`                                                       |
| App secret     | `notification-alerting-pg-app` (CNPG-managed; carries `username`, `password`, `uri`) |
| Backup secret  | `notification-alerting-pg-backup` (S3 credentials for barman-cloud)          |
...
`services.notification-alerting-service.envSecrets.DATABASE_URL`
