...
#[tokio::main]
async fn main() {
    observability::init_tracing("code-security-scanning-service");
    ...
    let public = Router::new().route(
        "/health",
        get(|| async { axum::Json(HealthStatus::ok("code-security-scanning-service")) }),
    );
    ...
    let protected = Router::new()
        ...
        .route(
            "/api/v1/code-security/scans/{id}",
            get(handlers::get_item),
        )
        .route(
            "/api/v1/code-security/scans/{id}/secondary",
    ...
    tracing::info!("starting code-security-scanning-service on {addr}");
