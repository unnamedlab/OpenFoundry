...
#[tokio::main]
async fn main() {
    observability::init_tracing("llm-catalog-service");
    ...
    let public = Router::new().route(
        "/health",
        get(|| async { axum::Json(HealthStatus::ok("llm-catalog-service")) }),
    );
    ...
    tracing::info!("starting llm-catalog-service on {addr}");
