...
| `backing_fs_local_round_trip.rs`, `backing_fs_logical_to_physical_mapping_stable.rs`, `presigned_download_url_audited.rs` | backing filesystem and presign audit | implemented for local backing FS, presigned download/upload-url audit, storage-details, and multipart upload; S3/HDFS remain config-gated |
...
- 2026-05-08 (DV-6, backing filesystem and upload parity): added `internal/backingfs` with a Rust-compatible local backing filesystem surface (`Put`/`Get`/`List`/`Delete`, deterministic `local:///base/logical` physical URIs, HMAC `expires`/`sig` local proxy URLs, and path-traversal protection). The handlers now emit injectable/slog audit events for `files.download` and `files.upload_url`, reject unsafe upload-url logical paths, and keep the existing local-proxy, storage-details, and multipart upload behavior under real configured `BackingFS` instances. Go tests now cover the Rust local round-trip, stable logical-to-physical mapping, presigned download audit, upload-url audit, storage-details totals/TTL, multipart local writes, and local proxy signature safety. Remaining config-gated items: S3 and HDFS backing drivers are intentionally not wired in Go yet because this repo currently exposes only the local Go `BackingFS` implementation and no Go S3/HDFS presigner configuration block equivalent; those drivers should be enabled after adding typed config and storage-abstraction implementations outside this slice.
