...
| Field          | Value                                                              |
|----------------|--------------------------------------------------------------------|
| Manifest       | [`infra/k8s/platform/manifests/cnpg/clusters/ontology-query-pg.yaml`](../../../infra/k8s/platform/manifests/cnpg/clusters/ontology-query-pg.yaml) |
| `kind`         | `Cluster` (`postgresql.cnpg.io/v1`, CloudNativePG / cloudnative-pg)|
| Cluster name   | `ontology-query-pg`                                                   |
| Namespace      | `openfoundry`                                                      |
| Read/write DNS | `ontology-query-pg-rw.openfoundry.svc.cluster.local:5432`             |
| Read-only DNS  | `ontology-query-pg-ro.openfoundry.svc.cluster.local:5432`             |
| Database       | `ontology_query`                                                       |
| App secret     | `ontology-query-pg-app` (CNPG-managed; carries `username`, `password`, `uri`) |
| Backup secret  | `ontology-query-pg-backup` (S3 credentials for barman-cloud)          |
...
`services.ontology-query-service.envSecrets.DATABASE_URL`
