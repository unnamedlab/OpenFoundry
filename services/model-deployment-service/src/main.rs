...
#[tokio::main]
async fn main() {
    observability::init_tracing("model-deployment-service");
    ...
    let public = Router::new().route(
        "/health",
        get(|| async { axum::Json(HealthStatus::ok("model-deployment-service")) }),
    );
    ...
    tracing::info!("starting model-deployment-service on {addr}");
