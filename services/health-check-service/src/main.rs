...
#[tokio::main]
async fn main() {
    observability::init_tracing("health-check-service");
    ...
    let public = Router::new().route(
        "/health",
        get(|| async { axum::Json(HealthStatus::ok("health-check-service")) }),
    );
    ...
    let protected = Router::new()
        ...
        .route(
            "/api/v1/health-checks/checks/{id}",
            get(handlers::get_item),
        )
        .route(
            "/api/v1/health-checks/checks/{id}/secondary",
    ...
    tracing::info!("starting health-check-service on {addr}");
