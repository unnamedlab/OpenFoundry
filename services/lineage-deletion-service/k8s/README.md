...
| Field          | Value                                                              |
|----------------|--------------------------------------------------------------------|
| Manifest       | [`infra/k8s/platform/manifests/cnpg/clusters/lineage-deletion-pg.yaml`](../../../infra/k8s/platform/manifests/cnpg/clusters/lineage-deletion-pg.yaml) |
| `kind`         | `Cluster` (`postgresql.cnpg.io/v1`, CloudNativePG / cloudnative-pg)|
| Cluster name   | `lineage-deletion-pg`                                                   |
| Namespace      | `openfoundry`                                                      |
| Read/write DNS | `lineage-deletion-pg-rw.openfoundry.svc.cluster.local:5432`             |
| Read-only DNS  | `lineage-deletion-pg-ro.openfoundry.svc.cluster.local:5432`             |
| Database       | `lineage_deletion`                                                       |
| App secret     | `lineage-deletion-pg-app` (CNPG-managed; carries `username`, `password`, `uri`) |
| Backup secret  | `lineage-deletion-pg-backup` (S3 credentials for barman-cloud)          |
...
`services.lineage-deletion-service.envSecrets.DATABASE_URL`
