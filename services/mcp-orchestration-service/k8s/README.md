...
| Field          | Value                                                              |
|----------------|--------------------------------------------------------------------|
| Manifest       | [`infra/k8s/platform/manifests/cnpg/clusters/mcp-orchestration-pg.yaml`](../../../infra/k8s/platform/manifests/cnpg/clusters/mcp-orchestration-pg.yaml) |
| `kind`         | `Cluster` (`postgresql.cnpg.io/v1`, CloudNativePG / cloudnative-pg)|
| Cluster name   | `mcp-orchestration-pg`                                                   |
| Namespace      | `openfoundry`                                                      |
| Read/write DNS | `mcp-orchestration-pg-rw.openfoundry.svc.cluster.local:5432`             |
| Read-only DNS  | `mcp-orchestration-pg-ro.openfoundry.svc.cluster.local:5432`             |
| Database       | `mcp_orchestration`                                                       |
| App secret     | `mcp-orchestration-pg-app` (CNPG-managed; carries `username`, `password`, `uri`) |
| Backup secret  | `mcp-orchestration-pg-backup` (S3 credentials for barman-cloud)          |
...
`services.mcp-orchestration-service.envSecrets.DATABASE_URL`
