...
#[tokio::main]
async fn main() {
    observability::init_tracing("telemetry-governance-service");
    ...
    let public = Router::new().route(
        "/health",
        get(|| async { axum::Json(HealthStatus::ok("telemetry-governance-service")) }),
    );
    ...
    let protected = Router::new()
        ...
        .route(
            "/api/v1/telemetry-governance/exports/{id}",
            get(handlers::get_item),
        )
        .route(
            "/api/v1/telemetry-governance/exports/{id}/secondary",
    ...
    tracing::info!("starting telemetry-governance-service on {addr}");
