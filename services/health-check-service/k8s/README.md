...
| Field          | Value                                                              |
|----------------|--------------------------------------------------------------------|
| Manifest       | [`infra/k8s/cnpg/clusters/health-check-pg.yaml`](../../../infra/k8s/cnpg/clusters/health-check-pg.yaml) |
| `kind`         | `Cluster` (`postgresql.cnpg.io/v1`, CloudNativePG / cloudnative-pg)|
| Cluster name   | `health-check-pg`                                                   |
| Namespace      | `openfoundry`                                                      |
| Read/write DNS | `health-check-pg-rw.openfoundry.svc.cluster.local:5432`             |
| Read-only DNS  | `health-check-pg-ro.openfoundry.svc.cluster.local:5432`             |
| Database       | `health_check`                                                       |
| App secret     | `health-check-pg-app` (CNPG-managed; carries `username`, `password`, `uri`) |
| Backup secret  | `health-check-pg-backup` (S3 credentials for barman-cloud)          |
...
`services.health-check-service.envSecrets.DATABASE_URL`
