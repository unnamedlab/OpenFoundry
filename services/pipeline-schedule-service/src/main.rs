...
#[tokio::main]
async fn main() {
    observability::init_tracing("pipeline-schedule-service");
    ...
    let public = Router::new().route(
        "/health",
        get(|| async { axum::Json(HealthStatus::ok("pipeline-schedule-service")) }),
    );
    ...
    tracing::info!("starting pipeline-schedule-service on {addr}");
