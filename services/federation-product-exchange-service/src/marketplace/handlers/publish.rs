...
pub async fn publish_version(
    ...
    let packaged_resources = serde_json::to_value(
        crate::marketplace::domain::devops::normalize_packaged_resources(&request.packaged_resources),
    )
    .map_err(|cause| internal_error(cause.to_string()))?;
	...
	.bind(crate::marketplace::domain::devops::normalize_release_channel(&request.release_channel))
...
fn ensure_action_id_matches(
    ...
    action_id: uuid::Uuid,
) -> Result<serde_json::Value, (axum::http::StatusCode, Json<crate::marketplace::handlers::ErrorResponse>)> {
