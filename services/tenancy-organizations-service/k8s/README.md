# `tenancy-organizations-service` — Kubernetes binding

This service runs as part of the umbrella Helm chart at
`infra/k8s/helm/open-foundry/`. The only state owned by this bounded
context is the **tenancy-organizations-service** Postgres schema (migrations under
...
| Field          | Value                                                              |
|----------------|--------------------------------------------------------------------|
| Manifest       | [`infra/k8s/platform/manifests/cnpg/clusters/tenancy-organizations-pg.yaml`](../../../infra/k8s/platform/manifests/cnpg/clusters/tenancy-organizations-pg.yaml) |
| `kind`         | `Cluster` (`postgresql.cnpg.io/v1`, CloudNativePG / cloudnative-pg)|
| Cluster name   | `tenancy-organizations-pg`                                                   |
| Namespace      | `openfoundry`                                                      |
| Read/write DNS | `tenancy-organizations-pg-rw.openfoundry.svc.cluster.local:5432`             |
| Read-only DNS  | `tenancy-organizations-pg-ro.openfoundry.svc.cluster.local:5432`             |
| Database       | `tenancy_organizations`                                                       |
| App secret     | `tenancy-organizations-pg-app` (CNPG-managed; carries `username`, `password`, `uri`) |
| Backup secret  | `tenancy-organizations-pg-backup` (S3 credentials for barman-cloud)          |
...
`services.tenancy-organizations-service.envSecrets.DATABASE_URL`
