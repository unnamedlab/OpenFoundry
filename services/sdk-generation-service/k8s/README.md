...
| Field          | Value                                                              |
|----------------|--------------------------------------------------------------------|
| Manifest       | [`infra/k8s/platform/manifests/cnpg/clusters/sdk-generation-pg.yaml`](../../../infra/k8s/platform/manifests/cnpg/clusters/sdk-generation-pg.yaml) |
| `kind`         | `Cluster` (`postgresql.cnpg.io/v1`, CloudNativePG / cloudnative-pg)|
| Cluster name   | `sdk-generation-pg`                                                   |
| Namespace      | `openfoundry`                                                      |
| Read/write DNS | `sdk-generation-pg-rw.openfoundry.svc.cluster.local:5432`             |
| Read-only DNS  | `sdk-generation-pg-ro.openfoundry.svc.cluster.local:5432`             |
| Database       | `sdk_generation`                                                       |
| App secret     | `sdk-generation-pg-app` (CNPG-managed; carries `username`, `password`, `uri`) |
| Backup secret  | `sdk-generation-pg-backup` (S3 credentials for barman-cloud)          |
...
`services.sdk-generation-service.envSecrets.DATABASE_URL`
