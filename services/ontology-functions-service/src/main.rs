//! `ontology-functions-service` binary entry point.
//!
//! Owns the function-runtime plane of the ontology
//! (`/api/v1/ontology/functions/*`) per
//! `docs/architecture/services-and-ports.md`. Business logic lives in
//! [`ontology_kernel::handlers::functions`]; this binary wires
...
//! owner of the routes the edge gateway already proxies to it
...
use ontology_kernel::{
    AppState, domain::pg_repository::PostgresDefinitionStore, handlers::functions,
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    tracing::info!(%addr, nats_url = %cfg.nats_url, "starting ontology-functions-service");
...
async fn build_stores(
    ...
    if let Ok(search) = search_backend_from_env() {
        ...
        tracing::warn!(
            "SEARCH_ENDPOINT/SEARCH_BACKEND not configured for ontology-functions-service; \
