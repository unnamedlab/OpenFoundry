# `ai-application-generation-service` — Kubernetes binding

This service runs as part of the umbrella Helm chart at
`infra/k8s/helm/open-foundry/`. The only state owned by this bounded
context is the **ai-application-generation-service** Postgres schema (migrations under
...
| Field          | Value                                                              |
|----------------|--------------------------------------------------------------------|
| Manifest       | [`infra/k8s/platform/manifests/cnpg/clusters/ai-application-generation-pg.yaml`](../../../infra/k8s/platform/manifests/cnpg/clusters/ai-application-generation-pg.yaml) |
| `kind`         | `Cluster` (`postgresql.cnpg.io/v1`, CloudNativePG / cloudnative-pg)|
| Cluster name   | `ai-application-generation-pg`                                                   |
| Namespace      | `openfoundry`                                                      |
| Read/write DNS | `ai-application-generation-pg-rw.openfoundry.svc.cluster.local:5432`             |
| Read-only DNS  | `ai-application-generation-pg-ro.openfoundry.svc.cluster.local:5432`             |
| Database       | `ai_application_generation`                                                       |
| App secret     | `ai-application-generation-pg-app` (CNPG-managed; carries `username`, `password`, `uri`) |
| Backup secret  | `ai-application-generation-pg-backup` (S3 credentials for barman-cloud)          |
...
`services.ai-application-generation-service.envSecrets.DATABASE_URL`
