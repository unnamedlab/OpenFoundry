...
use crate::virtual_table::{AppState, domain::egress::EgressPolicy};
...
pub async fn get(
    ...
    validate_via_network_boundary_service(state, &url, &policy).await?;
...
async fn validate_via_network_boundary_service(
    state: &AppState,
    url: &Url,
    policy: &EgressPolicy,
) -> Result<(), String> {
    let endpoint = Url::parse(&state.network_boundary_service_url)
        .map_err(|error| error.to_string())?
        .join("/internal/network-boundaries/egress/validate")
        .map_err(|error| error.to_string())?;
    let request = ValidateEgressRequest {
        url: url.to_string(),
        allowed_hosts: policy.allowed_hosts.clone(),
        blocked_hosts: policy.blocked_hosts.clone(),
    ...
    let response = state
        ...
        .send()
        .await
        .map_err(|error| format!("network-boundary-service validation failed: {error}"))?;
    let status = response.status();
    let payload = response
        .json::<ValidateEgressResponse>()
        .await
        .map_err(|error| format!("invalid network-boundary-service response: {error}"))?;

    if !status.is_success() {
        return Err(payload
            .reason
            .unwrap_or_else(|| format!("network-boundary-service returned HTTP {status}")));
    }
    if !payload.allowed {
        return Err(payload
            .reason
            .unwrap_or_else(|| format!("egress denied for '{}'", url)));
    }
