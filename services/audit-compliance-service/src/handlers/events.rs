...
use crate::{
    ...
    handlers::{ServiceResult, bad_request, db_error, internal_error, load_event_row, load_events},
    models::{
...
pub async fn persist_event(
    ...
    let retention_until = now + Duration::days(request.retention_days as i64);
    ...
    event.retention_until = retention_until;
    Ok(event)
}
