...
| Field          | Value                                                              |
|----------------|--------------------------------------------------------------------|
| Manifest       | [`infra/k8s/platform/manifests/cnpg/clusters/model-lifecycle-pg.yaml`](../../../infra/k8s/platform/manifests/cnpg/clusters/model-lifecycle-pg.yaml) |
| `kind`         | `Cluster` (`postgresql.cnpg.io/v1`, CloudNativePG / cloudnative-pg)|
| Cluster name   | `model-lifecycle-pg`                                                   |
| Namespace      | `openfoundry`                                                      |
| Read/write DNS | `model-lifecycle-pg-rw.openfoundry.svc.cluster.local:5432`             |
| Read-only DNS  | `model-lifecycle-pg-ro.openfoundry.svc.cluster.local:5432`             |
| Database       | `model_lifecycle`                                                       |
| App secret     | `model-lifecycle-pg-app` (CNPG-managed; carries `username`, `password`, `uri`) |
| Backup secret  | `model-lifecycle-pg-backup` (S3 credentials for barman-cloud)          |
...
`services.model-lifecycle-service.envSecrets.DATABASE_URL`
