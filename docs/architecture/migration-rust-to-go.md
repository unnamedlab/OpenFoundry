...
| 6 | ML/AI/apps & retire Rust | 🟡 in progress — `code-repository-review-service` ✅ foundation; `media-transform-runtime-service` ✅ foundation (REST runtime + 33-entry catalog verbatim; image-handler slice flips Native entries from not_implemented → native once Go-native handlers land). Inventory at openfoundry-go/INVENTORY-PHASE6.md. AI-kernel-bound shells (llm-catalog, model-{catalog,deployment}, agent-runtime, ai-evaluation, application-composition, solution-design, retrieval-context) blocked on libs/ai-kernel-go (~7515 LOC, multi-iteration). Architecture+slice candidates queued: entity-resolution, ontology-exploratory-analysis, reindex-coordinator, lineage, workflow-automation, federation-product-exchange. pyo3 sidecars (notebook-runtime, pipeline-build, ontology-actions) STOP-and-ask. |
