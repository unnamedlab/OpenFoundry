...
#[tokio::main]
async fn main() {
    observability::init_tracing("monitoring-rules-service");
    ...
    let public = Router::new().route(
        "/health",
        get(|| async { axum::Json(HealthStatus::ok("monitoring-rules-service")) }),
    );
    ...
    let protected = Router::new()
        ...
        .route(
            "/api/v1/monitoring/rules/{id}",
            get(handlers::get_item),
        )
        .route(
            "/api/v1/monitoring/rules/{id}/secondary",
    ...
    tracing::info!("starting monitoring-rules-service on {addr}");
