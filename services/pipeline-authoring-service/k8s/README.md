# `pipeline-authoring-service` — Kubernetes binding

This service runs as part of the umbrella Helm chart at
`infra/k8s/helm/open-foundry/`. The only state owned by this bounded
context is the **pipeline-authoring-service** Postgres schema (migrations under
...
| Read/write DNS | `pipeline-authoring-pg-rw.openfoundry.svc.cluster.local:5432`             |
| Read-only DNS  | `pipeline-authoring-pg-ro.openfoundry.svc.cluster.local:5432`             |
| Database       | `pipeline_authoring`                                                       |
| App secret     | `pipeline-authoring-pg-app` (CNPG-managed; carries `username`, `password`, `uri`) |
| Backup secret  | `pipeline-authoring-pg-backup` (S3 credentials for barman-cloud)          |
...
`services.pipeline-authoring-service.envSecrets.DATABASE_URL`
