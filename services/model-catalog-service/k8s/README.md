...
| Read/write DNS | `model-catalog-pg-rw.openfoundry.svc.cluster.local:5432`             |
| Read-only DNS  | `model-catalog-pg-ro.openfoundry.svc.cluster.local:5432`             |
| Database       | `model_catalog`                                                       |
| App secret     | `model-catalog-pg-app` (CNPG-managed; carries `username`, `password`, `uri`) |
| Backup secret  | `model-catalog-pg-backup` (S3 credentials for barman-cloud)          |
...
`services.model-catalog-service.envSecrets.DATABASE_URL`
