...
#[tokio::main]
async fn main() {
    observability::init_tracing("mcp-orchestration-service");
    ...
    let public = Router::new().route(
        "/health",
        get(|| async { axum::Json(HealthStatus::ok("mcp-orchestration-service")) }),
    );
    ...
    let protected = Router::new()
        ...
        .route(
            "/api/v1/mcp/servers/{id}/secondary",
    ...
    tracing::info!("starting mcp-orchestration-service on {addr}");
