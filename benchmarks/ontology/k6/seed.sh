...
#
# Estrategia: paginar `GET …/by-type/{type_id}` con `limit=500` y
...
while [[ $(wc -l <"$OUT") -lt $TARGET ]]; do
  url="$OF_BENCH_BASE_URL/api/v1/ontology/objects/$OF_BENCH_TENANT/by-type/$OF_BENCH_TYPE_ID?limit=500"
  if [[ -n "$cursor" ]]; then
    url+="&page_token=$cursor"
  fi
  resp=$(curl -fsS -H "Authorization: Bearer $OF_BENCH_TOKEN" -H 'X-Consistency: eventual' "$url")
  echo "$resp" | jq -r '.objects[].object_id' >> "$OUT"
  cursor=$(echo "$resp" | jq -r '.next_page_token // empty')
  if [[ -z "$cursor" ]]; then
    break
  fi
done
