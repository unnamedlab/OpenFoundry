...
#
# Canonical source for K8s deployments lives at
# `infra/k8s/platform/manifests/cnpg/clusters/bootstrap-sql/_common-outbox-schema.yaml`
# — keep the `outbox.events` / `outbox.heartbeat` DDL in sync. Compose
# does not provision per-service `svc_<bc>` roles or the `debezium_cdc`
# role, so the GRANTs from the K8s bootstrap are intentionally omitted
# here.
set -euo pipefail
