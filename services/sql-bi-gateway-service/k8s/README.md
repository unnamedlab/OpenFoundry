...
| Field          | Value                                                              |
|----------------|--------------------------------------------------------------------|
| Manifest       | [`infra/k8s/platform/manifests/cnpg/clusters/sql-bi-gateway-pg.yaml`](../../../infra/k8s/platform/manifests/cnpg/clusters/sql-bi-gateway-pg.yaml) |
| `kind`         | `Cluster` (`postgresql.cnpg.io/v1`, CloudNativePG / cloudnative-pg)|
| Cluster name   | `sql-bi-gateway-pg`                                                   |
| Namespace      | `openfoundry`                                                      |
| Read/write DNS | `sql-bi-gateway-pg-rw.openfoundry.svc.cluster.local:5432`             |
| Read-only DNS  | `sql-bi-gateway-pg-ro.openfoundry.svc.cluster.local:5432`             |
| Database       | `sql_bi_gateway`                                                       |
| App secret     | `sql-bi-gateway-pg-app` (CNPG-managed; carries `username`, `password`, `uri`) |
| Backup secret  | `sql-bi-gateway-pg-backup` (S3 credentials for barman-cloud)          |
...
`services.sql-bi-gateway-service.envSecrets.DATABASE_URL`
