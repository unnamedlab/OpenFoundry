# `workflow-automation-service` — Kubernetes binding

This service runs as part of the umbrella Helm chart at
`infra/k8s/helm/open-foundry/`. The only state owned by this bounded
context is the **workflow-automation-service** Postgres schema (migrations under
...
| Field          | Value                                                              |
|----------------|--------------------------------------------------------------------|
| Manifest       | [`infra/k8s/platform/manifests/cnpg/clusters/workflow-automation-pg.yaml`](../../../infra/k8s/platform/manifests/cnpg/clusters/workflow-automation-pg.yaml) |
| `kind`         | `Cluster` (`postgresql.cnpg.io/v1`, CloudNativePG / cloudnative-pg)|
| Cluster name   | `workflow-automation-pg`                                                   |
| Namespace      | `openfoundry`                                                      |
| Read/write DNS | `workflow-automation-pg-rw.openfoundry.svc.cluster.local:5432`             |
| Read-only DNS  | `workflow-automation-pg-ro.openfoundry.svc.cluster.local:5432`             |
| Database       | `workflow_automation`                                                       |
| App secret     | `workflow-automation-pg-app` (CNPG-managed; carries `username`, `password`, `uri`) |
| Backup secret  | `workflow-automation-pg-backup` (S3 credentials for barman-cloud)          |
...
`services.workflow-automation-service.envSecrets.DATABASE_URL`
