...
#[tokio::main]
async fn main() {
    observability::init_tracing("scenario-simulation-service");
    ...
    let public = Router::new().route(
        "/health",
        get(|| async { axum::Json(HealthStatus::ok("scenario-simulation-service")) }),
    );
    ...
    let protected = Router::new()
        ...
        .route(
            "/api/v1/scenarios/simulations/{id}",
            get(handlers::get_item),
        )
        .route(
            "/api/v1/scenarios/simulations/{id}/secondary",
    ...
    tracing::info!("starting scenario-simulation-service on {addr}");
