...
* `failureDomain` is set explicitly on every new pool. We use `zone` because
  ...
  `infra/k8s/platform/manifests/strimzi/kafka-cluster.yaml:67`); no `topology.rook.io/rack` (or
...
* Kafka (`infra/k8s/platform/manifests/strimzi/kafka-cluster.yaml`) and Postgres should be
