# `ontology-definition-service` — Kubernetes binding

This service runs as part of the umbrella Helm chart at
`infra/k8s/helm/open-foundry/`. The only state owned by this bounded
context is the **ontology-definition-service** Postgres schema (migrations under
...
| Field          | Value                                                              |
|----------------|--------------------------------------------------------------------|
| Manifest       | [`infra/k8s/platform/manifests/cnpg/clusters/ontology-definition-pg.yaml`](../../../infra/k8s/platform/manifests/cnpg/clusters/ontology-definition-pg.yaml) |
| `kind`         | `Cluster` (`postgresql.cnpg.io/v1`, CloudNativePG / cloudnative-pg)|
| Cluster name   | `ontology-definition-pg`                                                   |
| Namespace      | `openfoundry`                                                      |
| Read/write DNS | `ontology-definition-pg-rw.openfoundry.svc.cluster.local:5432`             |
| Read-only DNS  | `ontology-definition-pg-ro.openfoundry.svc.cluster.local:5432`             |
| Database       | `ontology_definition`                                                       |
| App secret     | `ontology-definition-pg-app` (CNPG-managed; carries `username`, `password`, `uri`) |
| Backup secret  | `ontology-definition-pg-backup` (S3 credentials for barman-cloud)          |
...
`services.ontology-definition-service.envSecrets.DATABASE_URL`
