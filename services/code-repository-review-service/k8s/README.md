# `code-repository-review-service` — Kubernetes binding

This service runs as part of the umbrella Helm chart at
`infra/k8s/helm/open-foundry/`. The only state owned by this bounded
context is the **code-repository-review-service** Postgres schema (migrations under
...
| Field          | Value                                                              |
|----------------|--------------------------------------------------------------------|
| Manifest       | [`infra/k8s/platform/manifests/cnpg/clusters/code-repository-review-pg.yaml`](../../../infra/k8s/platform/manifests/cnpg/clusters/code-repository-review-pg.yaml) |
| `kind`         | `Cluster` (`postgresql.cnpg.io/v1`, CloudNativePG / cloudnative-pg)|
| Cluster name   | `code-repository-review-pg`                                                   |
| Namespace      | `openfoundry`                                                      |
| Read/write DNS | `code-repository-review-pg-rw.openfoundry.svc.cluster.local:5432`             |
| Read-only DNS  | `code-repository-review-pg-ro.openfoundry.svc.cluster.local:5432`             |
| Database       | `code_repository_review`                                                       |
| App secret     | `code-repository-review-pg-app` (CNPG-managed; carries `username`, `password`, `uri`) |
| Backup secret  | `code-repository-review-pg-backup` (S3 credentials for barman-cloud)          |
...
`services.code-repository-review-service.envSecrets.DATABASE_URL`
