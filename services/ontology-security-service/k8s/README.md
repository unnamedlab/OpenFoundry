...
| Field          | Value                                                              |
|----------------|--------------------------------------------------------------------|
| Manifest       | [`infra/k8s/platform/manifests/cnpg/clusters/ontology-security-pg.yaml`](../../../infra/k8s/platform/manifests/cnpg/clusters/ontology-security-pg.yaml) |
| `kind`         | `Cluster` (`postgresql.cnpg.io/v1`, CloudNativePG / cloudnative-pg)|
| Cluster name   | `ontology-security-pg`                                                   |
| Namespace      | `openfoundry`                                                      |
| Read/write DNS | `ontology-security-pg-rw.openfoundry.svc.cluster.local:5432`             |
| Read-only DNS  | `ontology-security-pg-ro.openfoundry.svc.cluster.local:5432`             |
| Database       | `ontology_security`                                                       |
| App secret     | `ontology-security-pg-app` (CNPG-managed; carries `username`, `password`, `uri`) |
| Backup secret  | `ontology-security-pg-backup` (S3 credentials for barman-cloud)          |
...
`services.ontology-security-service.envSecrets.DATABASE_URL`
