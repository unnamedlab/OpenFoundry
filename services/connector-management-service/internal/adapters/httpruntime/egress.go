...
//     is true OR the target is loopback (loopback always gets a free pass).
//   - `blocked_hosts` overrides everything.
//   - Non-empty `allowed_hosts` enforces an allowlist; wildcards and
//     suffix matches mirror the Rust `host_matches` rules.
//
// Per-request overrides live under `connection.config.egress.{allowed_hosts,
// blocked_hosts, allow_private_networks, allow_insecure_http}`. When absent
// the [EgressPolicyFromState] inputs win.
package httpruntime
...
// EgressPolicy mirrors Rust's `EgressPolicy`. All slices are stored
...
// The state inputs are (in Rust order) `allowed_egress_hosts` and
// `allow_private_network_egress`; the per-request overrides come from the
...
// isPrivateIP mirrors Rust's `is_private_ip` exactly: matches 10/8,
// 172.16-31/12, 192.168/16, 127/8, 169.254/16 plus IPv6 loopback and
