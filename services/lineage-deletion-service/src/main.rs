...
#[tokio::main]
async fn main() {
    observability::init_tracing("lineage-deletion-service");
    ...
    let http_client = reqwest::Client::builder()
        ...
        .build()
        .expect("failed to build lineage deletion HTTP client");
    ...
    let state = AppState {
        ...
        http_client,
        lineage_service_url: cfg.lineage_service_url.clone(),
        dataset_service_url: cfg.dataset_service_url.clone(),
        audit_compliance_service_url: cfg.audit_compliance_service_url.clone(),
        storage,
    };

    let public = Router::new().route(
        "/health",
        get(|| async { axum::Json(HealthStatus::ok("lineage-deletion-service")) }),
    );

    let protected = Router::new()
        .route(
            "/api/v1/lineage-deletions",
            get(handlers::deletion::list_deletions).post(handlers::deletion::request_deletion),
        )
        .route(
            "/api/v1/audit/gdpr/erase",
            post(handlers::deletion::request_deletion),
        )
    ...
    tracing::info!("starting lineage-deletion-service on {addr}");
