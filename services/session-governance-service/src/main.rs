...
#[tokio::main]
async fn main() {
    observability::init_tracing("session-governance-service");
    ...
    let public = Router::new().route(
        "/health",
        get(|| async { axum::Json(HealthStatus::ok("session-governance-service")) }),
    );
    ...
    tracing::info!("starting session-governance-service on {addr}");
