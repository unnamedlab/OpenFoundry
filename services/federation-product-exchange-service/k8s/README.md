# `federation-product-exchange-service` — Kubernetes binding

This service runs as part of the umbrella Helm chart at
`infra/k8s/helm/open-foundry/`. The only state owned by this bounded
context is the **federation-product-exchange-service** Postgres schema (migrations under
...
| Field          | Value                                                              |
|----------------|--------------------------------------------------------------------|
| Manifest       | [`infra/k8s/platform/manifests/cnpg/clusters/federation-product-exchange-pg.yaml`](../../../infra/k8s/platform/manifests/cnpg/clusters/federation-product-exchange-pg.yaml) |
| `kind`         | `Cluster` (`postgresql.cnpg.io/v1`, CloudNativePG / cloudnative-pg)|
| Cluster name   | `federation-product-exchange-pg`                                                   |
| Namespace      | `openfoundry`                                                      |
| Read/write DNS | `federation-product-exchange-pg-rw.openfoundry.svc.cluster.local:5432`             |
| Read-only DNS  | `federation-product-exchange-pg-ro.openfoundry.svc.cluster.local:5432`             |
| Database       | `federation_product_exchange`                                                       |
| App secret     | `federation-product-exchange-pg-app` (CNPG-managed; carries `username`, `password`, `uri`) |
| Backup secret  | `federation-product-exchange-pg-backup` (S3 credentials for barman-cloud)          |
...
`services.federation-product-exchange-service.envSecrets.DATABASE_URL`
