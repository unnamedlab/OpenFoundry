...
| 6 | ML/AI/apps & retire Rust | 🟡 in progress — `code-repository-review-service` ✅ foundation; `media-transform-runtime-service` ✅ foundation; `entity-resolution-service` ✅ arch+slice; `ontology-exploratory-analysis-service` ✅ substrate-only; `reindex-coordinator-service` ✅ architecture+pure-logic-slice (event/state/scan/topics + UUID-v5 derivation pinned by tests; Kafka consumer + Cassandra scanner + JobRepo queued). Inventory at openfoundry-go/INVENTORY-PHASE6.md. AI-kernel-bound shells blocked on libs/ai-kernel-go (~7515 LOC, multi-iteration). Architecture+slice candidates queued: lineage, workflow-automation, federation-product-exchange. pyo3 sidecars (notebook-runtime, pipeline-build, ontology-actions) STOP-and-ask. |
