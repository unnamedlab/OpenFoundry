...
#[tokio::main]
async fn main() {
    observability::init_tracing("solution-design-service");
    ...
    let public = Router::new().route(
        "/health",
        get(|| async { axum::Json(HealthStatus::ok("solution-design-service")) }),
    );
    ...
    let protected = Router::new()
        ...
        .route(
            "/api/v1/solution-design/diagrams/{id}",
            get(handlers::get_item),
        )
        .route(
            "/api/v1/solution-design/diagrams/{id}/secondary",
    ...
    tracing::info!("starting solution-design-service on {addr}");
