# Vespa subchart — sizing & operations
...
the source tree at `infra/k8s/vespa/app/`. Updates are deployed by
the `Job` in [`templates/job-deploy.yaml`](templates/job-deploy.yaml)
which runs `vespa-deploy prepare && vespa-deploy activate`.

## Runbooks

* **Add a content node**: bump `content.replicas` in `values-prod.yaml`,
  `helm upgrade`. Vespa rebalances buckets automatically. Watch
...
* **Full cluster recovery**: redeploy `vespa.enabled=true`, then run
  the OntologyReindex workflow once per tenant.

## Dev / CI

`vespa.enabled=false` in [`values-dev.yaml`](../../values-dev.yaml).
