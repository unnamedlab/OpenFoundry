...
//! every minute — see [`crate::pipeline_schedule::domain::cron_registrar`] for the writer
//! side and the helm template
//! `infra/helm/apps/of-data-engine/templates/cronjob-pipeline-scheduler.yaml`
//! for the runner side.

use crate::pipeline_schedule::domain::trigger::{Schedule as ScheduleRow, Trigger, TriggerKind};
...
fn flavor_str(flavor: crate::pipeline_schedule::domain::trigger::CronFlavor) -> &'static str {
    match flavor {
        crate::pipeline_schedule::domain::trigger::CronFlavor::Unix5 => "unix5",
        crate::pipeline_schedule::domain::trigger::CronFlavor::Quartz6 => "quartz6",
    }
}
...
fn plan_for_trigger(trigger: &Trigger) -> CronEmitterPlan {
    match &trigger.kind {
        ...
        TriggerKind::Compound(c) => {
            ...
            let is_or = matches!(c.op, crate::pipeline_schedule::domain::trigger::CompoundOp::Or);
            if all_time && is_or {
                let clauses = c
                    .components
                    .iter()
...
mod tests {
    use super::*;
    use crate::pipeline_schedule::domain::trigger::{
    ...
    fn schedule_with(trigger: Trigger) -> ScheduleRow {
        ScheduleRow {
            ...
            active_run_id: None,
            scope_kind: crate::pipeline_schedule::domain::trigger::ScheduleScopeKind::User,
