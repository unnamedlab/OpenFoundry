...
| Read/write DNS | `data-asset-catalog-pg-rw.openfoundry.svc.cluster.local:5432`      |
| Read-only DNS  | `data-asset-catalog-pg-ro.openfoundry.svc.cluster.local:5432`      |
| Database       | `data_asset_catalog`                                               |
| App secret     | `data-asset-catalog-pg-app` (CNPG-managed; carries `username`, `password`, `uri`) |
| Backup secret  | `data-asset-catalog-pg-backup` (S3 credentials for barman-cloud)   |
...
`services.data-asset-catalog-service.envSecrets.DATABASE_URL`
