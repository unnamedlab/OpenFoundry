# `execution-observability-service` — Kubernetes binding

This service runs as part of the umbrella Helm chart at
`infra/k8s/helm/open-foundry/`. The only state owned by this bounded
context is the **execution-observability-service** Postgres schema (migrations under
...
| Field          | Value                                                              |
|----------------|--------------------------------------------------------------------|
| Manifest       | [`infra/k8s/platform/manifests/cnpg/clusters/execution-observability-pg.yaml`](../../../infra/k8s/platform/manifests/cnpg/clusters/execution-observability-pg.yaml) |
| `kind`         | `Cluster` (`postgresql.cnpg.io/v1`, CloudNativePG / cloudnative-pg)|
| Cluster name   | `execution-observability-pg`                                                   |
| Namespace      | `openfoundry`                                                      |
| Read/write DNS | `execution-observability-pg-rw.openfoundry.svc.cluster.local:5432`             |
| Read-only DNS  | `execution-observability-pg-ro.openfoundry.svc.cluster.local:5432`             |
| Database       | `execution_observability`                                                       |
| App secret     | `execution-observability-pg-app` (CNPG-managed; carries `username`, `password`, `uri`) |
| Backup secret  | `execution-observability-pg-backup` (S3 credentials for barman-cloud)          |
...
`services.execution-observability-service.envSecrets.DATABASE_URL`
