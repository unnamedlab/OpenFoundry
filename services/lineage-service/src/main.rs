...
#[tokio::main]
async fn main() {
    observability::init_tracing("lineage-service");
    ...
    let public = Router::new()
        .route(
            "/health",
            get(|| async { axum::Json(HealthStatus::ok("lineage-service")) }),
        )
        .route(
    ...
    let protected = Router::new()
        .route(
            "/api/v1/lineage/datasets/{dataset_id}",
            get(handlers::lineage::get_dataset_lineage),
        )
        .route(
    ...
    tracing::info!("starting lineage-service on {addr}");
