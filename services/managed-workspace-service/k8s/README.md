...
| Field          | Value                                                              |
|----------------|--------------------------------------------------------------------|
| Manifest       | [`infra/k8s/platform/manifests/cnpg/clusters/managed-workspace-pg.yaml`](../../../infra/k8s/platform/manifests/cnpg/clusters/managed-workspace-pg.yaml) |
| `kind`         | `Cluster` (`postgresql.cnpg.io/v1`, CloudNativePG / cloudnative-pg)|
| Cluster name   | `managed-workspace-pg`                                                   |
| Namespace      | `openfoundry`                                                      |
| Read/write DNS | `managed-workspace-pg-rw.openfoundry.svc.cluster.local:5432`             |
| Read-only DNS  | `managed-workspace-pg-ro.openfoundry.svc.cluster.local:5432`             |
| Database       | `managed_workspace`                                                       |
| App secret     | `managed-workspace-pg-app` (CNPG-managed; carries `username`, `password`, `uri`) |
| Backup secret  | `managed-workspace-pg-backup` (S3 credentials for barman-cloud)          |
...
`services.managed-workspace-service.envSecrets.DATABASE_URL`
