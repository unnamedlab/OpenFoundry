// Command action-log-sink is the `ontology.actions.applied.v1` →
// Iceberg `lakekeeper.default.action_log` consumer. Replaces the
// Scala `com.openfoundry.audit.ActionLogStreamSink` shipped in
// services/pipeline-runner-spark/. Phase B of ADR-0045.
package main
...
func buildWriter(cfg *config.Config) (writer.Writer, error) {
	...
	return writer.NewIcebergWriter(cfg.CatalogURL, cfg.TableWriterURL, cfg.Warehouse), nil
}
