...
mod tests {
    ...
    async fn append_writes_blob_and_metadata() {
        ...
        let writer = LegacyDatasetWriter::new(backend.clone(), "streaming_service");

        let snapshot = DatasetSnapshot::new("window_1", "snap_1", Bytes::from_static(b"row1"))
            .with_metadata(serde_json::json!({"rows": 1}));
        let outcome = writer.append(snapshot).await.unwrap();

        assert_eq!(outcome.backend, "legacy");
        assert!(backend.exists("streaming_service/window_1/snapshots/snap_1.bin").await.unwrap());
        assert!(backend.exists("streaming_service/window_1/snapshots/snap_1.json").await.unwrap());
    }

    #[tokio::test]
    async fn append_rejects_empty_identifiers() {
        let (_guard, backend) = fixture();
        let writer = LegacyDatasetWriter::new(backend, "streaming_service");
