...
| Field          | Value                                                              |
|----------------|--------------------------------------------------------------------|
| Manifest       | [`infra/k8s/platform/manifests/cnpg/clusters/cdc-metadata-pg.yaml`](../../../infra/k8s/platform/manifests/cnpg/clusters/cdc-metadata-pg.yaml) |
| `kind`         | `Cluster` (`postgresql.cnpg.io/v1`, CloudNativePG / cloudnative-pg)|
| Cluster name   | `cdc-metadata-pg`                                                   |
| Namespace      | `openfoundry`                                                      |
| Read/write DNS | `cdc-metadata-pg-rw.openfoundry.svc.cluster.local:5432`             |
| Read-only DNS  | `cdc-metadata-pg-ro.openfoundry.svc.cluster.local:5432`             |
| Database       | `cdc_metadata`                                                       |
| App secret     | `cdc-metadata-pg-app` (CNPG-managed; carries `username`, `password`, `uri`) |
| Backup secret  | `cdc-metadata-pg-backup` (S3 credentials for barman-cloud)          |
...
`services.cdc-metadata-service.envSecrets.DATABASE_URL`
