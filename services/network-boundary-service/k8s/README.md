...
| Field          | Value                                                              |
|----------------|--------------------------------------------------------------------|
| Manifest       | [`infra/k8s/platform/manifests/cnpg/clusters/network-boundary-pg.yaml`](../../../infra/k8s/platform/manifests/cnpg/clusters/network-boundary-pg.yaml) |
| `kind`         | `Cluster` (`postgresql.cnpg.io/v1`, CloudNativePG / cloudnative-pg)|
| Cluster name   | `network-boundary-pg`                                                   |
| Namespace      | `openfoundry`                                                      |
| Read/write DNS | `network-boundary-pg-rw.openfoundry.svc.cluster.local:5432`             |
| Read-only DNS  | `network-boundary-pg-ro.openfoundry.svc.cluster.local:5432`             |
| Database       | `network_boundary`                                                       |
| App secret     | `network-boundary-pg-app` (CNPG-managed; carries `username`, `password`, `uri`) |
| Backup secret  | `network-boundary-pg-backup` (S3 credentials for barman-cloud)          |
...
`services.network-boundary-service.envSecrets.DATABASE_URL`
