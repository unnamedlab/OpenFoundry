...
| Read/write DNS | `retention-policy-pg-rw.openfoundry.svc.cluster.local:5432`             |
| Read-only DNS  | `retention-policy-pg-ro.openfoundry.svc.cluster.local:5432`             |
| Database       | `retention_policy`                                                       |
| App secret     | `retention-policy-pg-app` (CNPG-managed; carries `username`, `password`, `uri`) |
| Backup secret  | `retention-policy-pg-backup` (S3 credentials for barman-cloud)          |
...
`services.retention-policy-service.envSecrets.DATABASE_URL`
