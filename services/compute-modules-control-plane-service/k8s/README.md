# `compute-modules-control-plane-service` — Kubernetes binding

This service runs as part of the umbrella Helm chart at
`infra/k8s/helm/open-foundry/`. The only state owned by this bounded
context is the **compute-modules-control-plane-service** Postgres schema (migrations under
...
| Field          | Value                                                              |
|----------------|--------------------------------------------------------------------|
| Manifest       | [`infra/k8s/platform/manifests/cnpg/clusters/compute-modules-control-plane-pg.yaml`](../../../infra/k8s/platform/manifests/cnpg/clusters/compute-modules-control-plane-pg.yaml) |
| `kind`         | `Cluster` (`postgresql.cnpg.io/v1`, CloudNativePG / cloudnative-pg)|
| Cluster name   | `compute-modules-control-plane-pg`                                                   |
| Namespace      | `openfoundry`                                                      |
| Read/write DNS | `compute-modules-control-plane-pg-rw.openfoundry.svc.cluster.local:5432`             |
| Read-only DNS  | `compute-modules-control-plane-pg-ro.openfoundry.svc.cluster.local:5432`             |
| Database       | `compute_modules_control_plane`                                                       |
| App secret     | `compute-modules-control-plane-pg-app` (CNPG-managed; carries `username`, `password`, `uri`) |
| Backup secret  | `compute-modules-control-plane-pg-backup` (S3 credentials for barman-cloud)          |
...
`services.compute-modules-control-plane-service.envSecrets.DATABASE_URL`
