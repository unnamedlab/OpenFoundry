//! Simple reconcile loop. Periodically scans the `ingest_jobs` table for
//! pending or materialised rows and re-applies their rendered resources to
//! the cluster (server-side apply is idempotent, so this is safe and cheap).
//!
...
//! human (or another controller) deletes them out from under us.
        ...
        match render_resources(&job.spec) {
            Ok(rendered) => {
                if let Err(err) = apply_resources(client, &rendered).await {
                    ...
                    let _ = repository::mark_failed(pool, job.id, &err.to_string()).await;
                } else {
                    let kc = rendered
                        .kafka_connector
                        .metadata
