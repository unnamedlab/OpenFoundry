...
#[tokio::main]
async fn main() {
    observability::init_tracing("custom-endpoints-service");
    ...
    let public = Router::new().route(
        "/health",
        get(|| async { axum::Json(HealthStatus::ok("custom-endpoints-service")) }),
    );
    ...
    let protected = Router::new()
        ...
        .route(
            "/api/v1/custom-endpoints/sets/{id}",
            get(handlers::get_item),
        )
        .route(
            "/api/v1/custom-endpoints/sets/{id}/secondary",
    ...
    tracing::info!("starting custom-endpoints-service on {addr}");
