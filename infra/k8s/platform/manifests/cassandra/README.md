...
the convention used by [`infra/k8s/platform/manifests/strimzi`](../strimzi/README.md) and
[`infra/k8s/platform/manifests/rook`](../rook/README.md): the upstream operator is installed
...
[`infra/k8s/platform/manifests/rook/cluster.yaml`](../rook/cluster.yaml) (NVMe-only OSDs,
...
helm install k8ssandra-operator \
  ...
  --create-namespace \
  -f infra/k8s/platform/manifests/cassandra/values-k8ssandra-operator.yaml
...
kubectl apply -f infra/k8s/platform/manifests/cassandra/cluster-dev.yaml
...
# kubectl apply -f infra/k8s/platform/manifests/cassandra/cluster-prod.yaml
```
...
- **Backups** are taken by Medusa to a Ceph S3 bucket (`cassandra-backups`,
  see [`infra/k8s/platform/manifests/rook/bucket.yaml`](../rook/bucket.yaml) for the
