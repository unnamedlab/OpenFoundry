...
use scheduling_linter::{
    Action, Finding, RuleId, Severity, SweepReport, planner::AppliedAction,
};
use uuid::Uuid;
...
async fn applying_a_pause_finding_pauses_the_schedule_row() {
    ...
    let AppliedAction { schedule_rid, action, .. } = plan[0].clone();
    assert_eq!(action, Action::Pause);
    schedule_store::set_paused(&pool, &schedule_rid, true, Some("LINTER"))
        .await
        .unwrap();

    let after = schedule_store::get_by_rid(&pool, &schedule.rid).await.unwrap();
    assert!(after.paused);
