...
context is the **event-streaming-service** Postgres schema (migrations under
...
| Field          | Value                                                              |
|----------------|--------------------------------------------------------------------|
| Manifest       | [`infra/k8s/cnpg/clusters/event-streaming-pg.yaml`](../../../infra/k8s/cnpg/clusters/event-streaming-pg.yaml) |
| `kind`         | `Cluster` (`postgresql.cnpg.io/v1`, CloudNativePG / cloudnative-pg)|
| Cluster name   | `event-streaming-pg`                                                   |
| Namespace      | `openfoundry`                                                      |
| Read/write DNS | `event-streaming-pg-rw.openfoundry.svc.cluster.local:5432`             |
| Read-only DNS  | `event-streaming-pg-ro.openfoundry.svc.cluster.local:5432`             |
| Database       | `event_streaming`                                                       |
| App secret     | `event-streaming-pg-app` (CNPG-managed; carries `username`, `password`, `uri`) |
| Backup secret  | `event-streaming-pg-backup` (S3 credentials for barman-cloud)          |
...
`services.event-streaming-service.envSecrets.DATABASE_URL`
