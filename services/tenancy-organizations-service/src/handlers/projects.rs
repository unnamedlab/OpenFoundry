...
pub async fn list_projects(
    ...
    let accessible = match list_accessible_projects(&state.ontology_db, &claims).await {
...
pub async fn get_project(
    ...
    if let Err(error) = ensure_project_view_access(&state.ontology_db, &claims, id).await {
...
pub async fn bind_project_resource(
    ...
    let Some(owner_id) =
        (match load_resource_owner_id(&state.ontology_db, resource_kind, body.resource_id).await {
    ...
    let existing_project_id =
        match load_resource_project_id(&state.ontology_db, resource_kind, body.resource_id).await {
    ...
    if let Err(error) =
        ensure_resource_manage_access(&state.ontology_db, &claims, owner_id, existing_project_id)
            .await
    {
        return forbidden(error);
    }
...
pub async fn unbind_project_resource(
    ...
    let Some(owner_id) =
        (match load_resource_owner_id(&state.ontology_db, resource_kind, resource_id).await {
            Ok(owner_id) => owner_id,
            Err(error) => return db_error(error),
        })
    else {
    ...
    if let Err(error) = ensure_resource_manage_access(
        &state.ontology_db,
        &claims,
        owner_id,
