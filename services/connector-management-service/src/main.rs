...
#[tokio::main]
async fn main() {
    observability::init_tracing("connector-management-service");
    ...
    let public = Router::new().route(
        "/health",
        get(|| async { axum::Json(HealthStatus::ok("connector-management-service")) }),
    );
    ...
    tracing::info!("starting connector-management-service on {addr}");
