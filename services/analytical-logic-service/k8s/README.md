...
| Field          | Value                                                              |
|----------------|--------------------------------------------------------------------|
| Manifest       | [`infra/k8s/platform/manifests/cnpg/clusters/analytical-logic-pg.yaml`](../../../infra/k8s/platform/manifests/cnpg/clusters/analytical-logic-pg.yaml) |
| `kind`         | `Cluster` (`postgresql.cnpg.io/v1`, CloudNativePG / cloudnative-pg)|
| Cluster name   | `analytical-logic-pg`                                                   |
| Namespace      | `openfoundry`                                                      |
| Read/write DNS | `analytical-logic-pg-rw.openfoundry.svc.cluster.local:5432`             |
| Read-only DNS  | `analytical-logic-pg-ro.openfoundry.svc.cluster.local:5432`             |
| Database       | `analytical_logic`                                                       |
| App secret     | `analytical-logic-pg-app` (CNPG-managed; carries `username`, `password`, `uri`) |
| Backup secret  | `analytical-logic-pg-backup` (S3 credentials for barman-cloud)          |
...
`services.analytical-logic-service.envSecrets.DATABASE_URL`
