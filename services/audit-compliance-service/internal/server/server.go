...
func New(cfg *config.Config, jwt *authmw.JWTConfig, sub *Subsystems, m *observability.Metrics, probes ...capabilities.DependencyProbe) *http.Server {
	...
	r.Route("/api/v1", func(api chi.Router) {
		...
		if sub.Audit != nil {
			...
			api.Get("/audit/monitoring/starter-pack", sub.Audit.GetAuditMonitoringStarterPack)
		...
		if sub.Retention != nil {
			...
			api.Get("/retention/executions", sub.Retention.ListExecutions)
			api.Post("/retention/executions", sub.Retention.RunExecutionHandler)
