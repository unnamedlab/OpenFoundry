...
#[tokio::main]
async fn main() {
    observability::init_tracing("compute-modules-runtime-service");
    ...
    let public = Router::new().route(
        "/health",
        get(|| async { axum::Json(HealthStatus::ok("compute-modules-runtime-service")) }),
    );
    ...
    let protected = Router::new()
        ...
        .route(
            "/api/v1/compute-modules/runs/{id}",
            get(handlers::get_item),
        )
        .route(
            "/api/v1/compute-modules/runs/{id}/secondary",
    ...
    tracing::info!("starting compute-modules-runtime-service on {addr}");
