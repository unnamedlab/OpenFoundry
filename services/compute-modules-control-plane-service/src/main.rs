...
#[tokio::main]
async fn main() {
    observability::init_tracing("compute-modules-control-plane-service");
    ...
    let public = Router::new().route(
        "/health",
        get(|| async { axum::Json(HealthStatus::ok("compute-modules-control-plane-service")) }),
    );
    ...
    let protected = Router::new()
        ...
        .route(
            "/api/v1/compute-modules/modules/{id}",
            get(handlers::get_item),
        )
        .route(
            "/api/v1/compute-modules/modules/{id}/secondary",
    ...
    tracing::info!("starting compute-modules-control-plane-service on {addr}");
