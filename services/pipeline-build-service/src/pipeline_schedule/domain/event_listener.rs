...
//! [`crate::pipeline_schedule::domain::cron_registrar`] (Tarea 3.5); here we only
...
use crate::pipeline_schedule::domain::trigger::{EventType, Schedule};
use crate::pipeline_schedule::domain::trigger_engine::{ObservedEvent, PgTriggerEvaluator, TriggerOutcome};
...
pub async fn find_schedules_referencing_target(
    ...
    let sql = format!(
        ...
        crate::pipeline_schedule::domain::schedule_store::SCHEDULE_COLUMNS,
    );
    ...
    for row in rows {
        ...
        let scope_kind = crate::pipeline_schedule::domain::trigger::ScheduleScopeKind::parse(&scope_kind_str)
            .unwrap_or(crate::pipeline_schedule::domain::trigger::ScheduleScopeKind::User);
        out.push(Schedule {
            id: row.try_get("id")?,
            rid,
