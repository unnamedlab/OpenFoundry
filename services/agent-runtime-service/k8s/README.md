...
| Field          | Value                                                              |
|----------------|--------------------------------------------------------------------|
| Manifest       | [`infra/k8s/platform/manifests/cnpg/clusters/agent-runtime-pg.yaml`](../../../infra/k8s/platform/manifests/cnpg/clusters/agent-runtime-pg.yaml) |
| `kind`         | `Cluster` (`postgresql.cnpg.io/v1`, CloudNativePG / cloudnative-pg)|
| Cluster name   | `agent-runtime-pg`                                                   |
| Namespace      | `openfoundry`                                                      |
| Read/write DNS | `agent-runtime-pg-rw.openfoundry.svc.cluster.local:5432`             |
| Read-only DNS  | `agent-runtime-pg-ro.openfoundry.svc.cluster.local:5432`             |
| Database       | `agent_runtime`                                                       |
| App secret     | `agent-runtime-pg-app` (CNPG-managed; carries `username`, `password`, `uri`) |
| Backup secret  | `agent-runtime-pg-backup` (S3 credentials for barman-cloud)          |
...
`services.agent-runtime-service.envSecrets.DATABASE_URL`
