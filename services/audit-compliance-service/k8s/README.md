...
| Field          | Value                                                              |
|----------------|--------------------------------------------------------------------|
| Manifest       | [`infra/k8s/platform/manifests/cnpg/clusters/audit-compliance-pg.yaml`](../../../infra/k8s/platform/manifests/cnpg/clusters/audit-compliance-pg.yaml) |
| `kind`         | `Cluster` (`postgresql.cnpg.io/v1`, CloudNativePG / cloudnative-pg)|
| Cluster name   | `audit-compliance-pg`                                                   |
| Namespace      | `openfoundry`                                                      |
| Read/write DNS | `audit-compliance-pg-rw.openfoundry.svc.cluster.local:5432`             |
| Read-only DNS  | `audit-compliance-pg-ro.openfoundry.svc.cluster.local:5432`             |
| Database       | `audit_compliance`                                                       |
| App secret     | `audit-compliance-pg-app` (CNPG-managed; carries `username`, `password`, `uri`) |
| Backup secret  | `audit-compliance-pg-backup` (S3 credentials for barman-cloud)          |
...
`services.audit-compliance-service.envSecrets.DATABASE_URL`
