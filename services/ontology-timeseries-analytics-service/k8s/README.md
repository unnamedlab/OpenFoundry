# `ontology-timeseries-analytics-service` — Kubernetes binding

This service runs as part of the umbrella Helm chart at
`infra/k8s/helm/open-foundry/`. The only state owned by this bounded
context is the **ontology-timeseries-analytics-service** Postgres schema (migrations under
...
| Field          | Value                                                              |
|----------------|--------------------------------------------------------------------|
| Manifest       | [`infra/k8s/platform/manifests/cnpg/clusters/ontology-timeseries-analytics-pg.yaml`](../../../infra/k8s/platform/manifests/cnpg/clusters/ontology-timeseries-analytics-pg.yaml) |
| `kind`         | `Cluster` (`postgresql.cnpg.io/v1`, CloudNativePG / cloudnative-pg)|
| Cluster name   | `ontology-timeseries-analytics-pg`                                                   |
| Namespace      | `openfoundry`                                                      |
| Read/write DNS | `ontology-timeseries-analytics-pg-rw.openfoundry.svc.cluster.local:5432`             |
| Read-only DNS  | `ontology-timeseries-analytics-pg-ro.openfoundry.svc.cluster.local:5432`             |
| Database       | `ontology_timeseries_analytics`                                                       |
| App secret     | `ontology-timeseries-analytics-pg-app` (CNPG-managed; carries `username`, `password`, `uri`) |
| Backup secret  | `ontology-timeseries-analytics-pg-backup` (S3 credentials for barman-cloud)          |
...
`services.ontology-timeseries-analytics-service.envSecrets.DATABASE_URL`
