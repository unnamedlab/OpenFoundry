...
| Field          | Value                                                              |
|----------------|--------------------------------------------------------------------|
| Manifest       | [`infra/k8s/platform/manifests/cnpg/clusters/lineage-pg.yaml`](../../../infra/k8s/platform/manifests/cnpg/clusters/lineage-pg.yaml) |
| `kind`         | `Cluster` (`postgresql.cnpg.io/v1`, CloudNativePG / cloudnative-pg)|
| Cluster name   | `lineage-pg`                                                   |
| Namespace      | `openfoundry`                                                      |
| Read/write DNS | `lineage-pg-rw.openfoundry.svc.cluster.local:5432`             |
| Read-only DNS  | `lineage-pg-ro.openfoundry.svc.cluster.local:5432`             |
| Database       | `lineage`                                                       |
| App secret     | `lineage-pg-app` (CNPG-managed; carries `username`, `password`, `uri`) |
| Backup secret  | `lineage-pg-backup` (S3 credentials for barman-cloud)          |
...
`services.lineage-service.envSecrets.DATABASE_URL`
