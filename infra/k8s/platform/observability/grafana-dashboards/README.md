...
| Vespa             | *Vespa Detailed Monitoring Dashboard*           | **[18308][gc-18308]** | Prometheus | Vespa-team-maintained dashboard for the metrics-proxy consumer used by `infra/k8s/platform/charts/vespa/templates/servicemonitor.yaml`. |
...
| CloudNativePG     | *CNPG cluster fleet*                            | n/a                 | Prometheus | OpenFoundry-specific fleet view across the per-bounded-context clusters in `infra/k8s/platform/manifests/cnpg/clusters/` — see [`cnpg-overview.json`](./cnpg-overview.json). Pairs with `cnpg.yaml`. |
...
These map 1:1 to the dashboards listed in ADR-0012 §4. All six are
...
|---|---|---|---|
| Data Plane SLO Overview              | `dp-slo-overview`  | aggregates the five active SLIs | [`dp-slo-overview.json`](./dp-slo-overview.json) |
...
| Vespa hybrid query SLO               | `dp-slo-vespa`     | §2.4 | [`dp-slo-vespa.json`](./dp-slo-vespa.json) |
| NATS control event SLO               | `dp-slo-nats`      | §2.5 | [`dp-slo-nats.json`](./dp-slo-nats.json) |
...
```bash
jq . infra/k8s/platform/observability/grafana-dashboards/*.json > /dev/null
```
