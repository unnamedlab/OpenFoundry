...
#[tokio::main]
async fn main() {
    observability::init_tracing("pipeline-build-service");
    ...
    let public = Router::new().route(
        "/health",
        get(|| async { axum::Json(HealthStatus::ok("pipeline-build-service")) }),
    );

    let protected = Router::new()
        .route(
            "/api/v1/pipelines/{id}/run",
            post(handlers::execute::trigger_run),
        )
        .route(
    ...
    tracing::info!("starting pipeline-build-service on {addr}");
