...
/// A unit of materialization (a dataset snapshot, a transaction commit, or a
/// view refresh) that needs to be persisted by a [`DatasetWriter`].
#[derive(Debug, Clone)]
pub struct DatasetSnapshot {
    /// Logical table this snapshot belongs to (e.g. `dataset_<uuid>`).
    pub table: String,
    ...
    /// Encoded payload (e.g. parquet bytes, JSON lines, ...). Opaque to the
    /// writer.
    pub payload: Bytes,
    /// Side metadata recorded next to the snapshot. The Iceberg writer
    /// forwards this to the catalog as snapshot summary properties; the
    /// legacy writer persists it as a sibling JSON file.
...
/// writer can be shared across handlers.
