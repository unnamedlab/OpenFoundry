# `compute-modules-runtime-service` — Kubernetes binding

This service runs as part of the umbrella Helm chart at
`infra/k8s/helm/open-foundry/`. The only state owned by this bounded
context is the **compute-modules-runtime-service** Postgres schema (migrations under
...
| Field          | Value                                                              |
|----------------|--------------------------------------------------------------------|
| Manifest       | [`infra/k8s/platform/manifests/cnpg/clusters/compute-modules-runtime-pg.yaml`](../../../infra/k8s/platform/manifests/cnpg/clusters/compute-modules-runtime-pg.yaml) |
| `kind`         | `Cluster` (`postgresql.cnpg.io/v1`, CloudNativePG / cloudnative-pg)|
| Cluster name   | `compute-modules-runtime-pg`                                                   |
| Namespace      | `openfoundry`                                                      |
| Read/write DNS | `compute-modules-runtime-pg-rw.openfoundry.svc.cluster.local:5432`             |
| Read-only DNS  | `compute-modules-runtime-pg-ro.openfoundry.svc.cluster.local:5432`             |
| Database       | `compute_modules_runtime`                                                       |
| App secret     | `compute-modules-runtime-pg-app` (CNPG-managed; carries `username`, `password`, `uri`) |
| Backup secret  | `compute-modules-runtime-pg-backup` (S3 credentials for barman-cloud)          |
...
`services.compute-modules-runtime-service.envSecrets.DATABASE_URL`
