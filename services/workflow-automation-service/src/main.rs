...
async fn main() {
    ...
    let protected = Router::new()
        ...
        .route(
            "/api/v1/workflow-automations/definitions/{id}",
            get(handlers::crud::get_workflow)
                .patch(handlers::crud::update_workflow)
                .delete(handlers::crud::delete_workflow),
        )
        .route(
            "/api/v1/workflow-automations/definitions/{id}/runs",
            get(handlers::runs::list_runs),
        )
        .route(
            "/api/v1/workflow-automations/definitions/{id}/runs/manual",
            post(handlers::execute::start_manual_run),
        )
    ...
    tracing::info!("starting workflow-automation-service on {addr}");
