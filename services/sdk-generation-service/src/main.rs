...
#[tokio::main]
async fn main() {
    observability::init_tracing("sdk-generation-service");
    ...
    let public = Router::new().route(
        "/health",
        get(|| async { axum::Json(HealthStatus::ok("sdk-generation-service")) }),
    );
    ...
    let protected = Router::new()
        ...
        .route(
            "/api/v1/sdk-generation/jobs/{id}",
            get(handlers::get_item),
        )
        .route(
            "/api/v1/sdk-generation/jobs/{id}/secondary",
    ...
    tracing::info!("starting sdk-generation-service on {addr}");
