...
#[tokio::main]
async fn main() {
    observability::init_tracing("spreadsheet-computation-service");
    ...
    let public = Router::new().route(
        "/health",
        get(|| async { axum::Json(HealthStatus::ok("spreadsheet-computation-service")) }),
    );
    ...
    let protected = Router::new()
        ...
        .route(
            "/api/v1/spreadsheets/sheets/{id}",
            get(handlers::get_item),
        )
        .route(
            "/api/v1/spreadsheets/sheets/{id}/secondary",
    ...
    tracing::info!("starting spreadsheet-computation-service on {addr}");
