...
mod tests {
    ...
    async fn append_writes_data_file_and_commits_snapshot() {
        ...
        assert_eq!(outcome.location, "iceberg://dataset_service/ds_42#snap_001");
        assert!(
            backend
                .exists("dataset_service/ds_42/data/snap_001.parquet")
                .await
                .unwrap()
        );
    ...
    async fn append_is_idempotent_per_snapshot_id_at_storage_layer() {
        ...
        let (_guard, backend, catalog) = fixture();
        let writer = IcebergDatasetWriter::new(backend, catalog.clone(), "dataset_service");

        let snap = DatasetSnapshot::new("ds_x", "snap_1", Bytes::from_static(b"v1"));
        writer.append(snap.clone()).await.unwrap();
        writer.append(snap).await.unwrap();

        let commits = catalog.snapshots(&IcebergTableRef::new("dataset_service", "ds_x"));
    ...
    async fn append_rejects_empty_identifiers() {
        let (_guard, backend, catalog) = fixture();
        let writer = IcebergDatasetWriter::new(backend, catalog, "dataset_service");
        let snap = DatasetSnapshot::new("ds_x", "", Bytes::new());
        assert!(matches!(
