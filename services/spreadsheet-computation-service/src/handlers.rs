...
pub async fn list_items(State(state): State<AppState>) -> impl IntoResponse {
    match sqlx::query_as::<_, PrimaryItem>(
        "SELECT * FROM spreadsheet_sheets ORDER BY created_at DESC LIMIT 200",
    )
    .fetch_all(&state.db)
    .await
    {
...
pub async fn create_item(
    ...
    let id = Uuid::now_v7();
    match sqlx::query_as::<_, PrimaryItem>(
        "INSERT INTO spreadsheet_sheets (id, payload) VALUES ($1, $2) RETURNING *",
    )
    .bind(id)
    .bind(&body.payload)
    .fetch_one(&state.db)
...
pub async fn get_item(
    ...
    match sqlx::query_as::<_, PrimaryItem>(
        "SELECT * FROM spreadsheet_sheets WHERE id = $1",
    )
    .bind(id)
...
pub async fn list_secondary(
    ...
    match sqlx::query_as::<_, SecondaryItem>(
        "SELECT * FROM spreadsheet_recalcs WHERE parent_id = $1 ORDER BY created_at DESC LIMIT 200",
    )
    .bind(parent_id)
    .fetch_all(&state.db)
    .await
...
pub async fn create_secondary(
    ...
    let id = Uuid::now_v7();
    match sqlx::query_as::<_, SecondaryItem>(
        "INSERT INTO spreadsheet_recalcs (id, parent_id, payload) VALUES ($1, $2, $3) RETURNING *",
    )
    .bind(id)
    .bind(parent_id)
    .bind(&body.payload)
