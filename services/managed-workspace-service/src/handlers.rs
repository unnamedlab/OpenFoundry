...
pub async fn list_items(State(state): State<AppState>) -> impl IntoResponse {
    match sqlx::query_as::<_, PrimaryItem>(
        "SELECT * FROM managed_workspaces ORDER BY created_at DESC LIMIT 200",
    )
    .fetch_all(&state.db)
    .await
    {
...
pub async fn create_item(
    ...
    let id = Uuid::now_v7();
    match sqlx::query_as::<_, PrimaryItem>(
        "INSERT INTO managed_workspaces (id, payload) VALUES ($1, $2) RETURNING *",
    )
    .bind(id)
    .bind(&body.payload)
    .fetch_one(&state.db)
...
pub async fn list_secondary(
    ...
    match sqlx::query_as::<_, SecondaryItem>(
        "SELECT * FROM managed_workspace_aliases WHERE parent_id = $1 ORDER BY created_at DESC LIMIT 200",
    )
    .bind(parent_id)
    .fetch_all(&state.db)
    .await
...
pub async fn create_secondary(
    ...
    let id = Uuid::now_v7();
    match sqlx::query_as::<_, SecondaryItem>(
        "INSERT INTO managed_workspace_aliases (id, parent_id, payload) VALUES ($1, $2, $3) RETURNING *",
    )
    .bind(id)
    .bind(parent_id)
    .bind(&body.payload)
