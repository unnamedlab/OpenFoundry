...
#[tokio::main]
async fn main() {
    observability::init_tracing("conversation-state-service");
    ...
    let public = Router::new().route(
        "/health",
        get(|| async { axum::Json(HealthStatus::ok("conversation-state-service")) }),
    );
    ...
    tracing::info!("starting conversation-state-service on {addr}");
