...
#[tokio::main]
async fn main() {
    observability::init_tracing("developer-console-service");
    ...
    let public = Router::new().route(
        "/health",
        get(|| async { axum::Json(HealthStatus::ok("developer-console-service")) }),
    );
    ...
    let protected = Router::new()
        ...
        .route(
            "/api/v1/developer-console/applications/{id}",
            get(handlers::get_item),
        )
        .route(
            "/api/v1/developer-console/applications/{id}/secondary",
    ...
    tracing::info!("starting developer-console-service on {addr}");
