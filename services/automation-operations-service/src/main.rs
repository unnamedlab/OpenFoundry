...
#[tokio::main]
async fn main() {
    observability::init_tracing("automation-operations-service");
    ...
    let public = Router::new().route(
        "/health",
        get(|| async { axum::Json(HealthStatus::ok("automation-operations-service")) }),
    );
    ...
    let protected = Router::new()
        ...
        .route(
            "/api/v1/automation-ops/queues/{id}",
            get(handlers::get_item),
        )
        .route(
            "/api/v1/automation-ops/queues/{id}/secondary",
    ...
    tracing::info!("starting automation-operations-service on {addr}");
