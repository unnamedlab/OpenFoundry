//! `event_streaming` — the streaming control plane absorbed from the
//! retired `event-streaming-service` per ADR-0030 (S8 merge).
//!
//! Exposes the building blocks of the streaming control plane so they can
//! be exercised both from the host binary and from the unit-test harness.
//!
//! The streaming domain has two faces:
...
pub struct AppState {
    ...
    /// ready handle.
    #[allow(dead_code)]
    pub backends: Arc<crate::event_streaming::backends::BackendRegistry>,
    /// Compiled routing table (`topic-routes.yaml`).
    #[allow(dead_code)]
    pub table: Arc<crate::event_streaming::router::RouteTable>,
    /// Prometheus metrics registry shared between the gRPC and REST
    /// listeners.
    #[allow(dead_code)]
    pub metrics: Arc<crate::event_streaming::metrics::Metrics>,
    /// Iceberg / legacy dataset writer used by topology runs.
    #[allow(dead_code)]
    pub dataset_writer: Arc<dyn crate::event_streaming::storage::DatasetWriter>,
    ...
    /// a [`crate::event_streaming::domain::hot_buffer::NoopHotBuffer`].
    pub hot_buffer: Arc<dyn crate::event_streaming::domain::hot_buffer::HotBuffer>,
    ...
    pub runtime_store: Arc<dyn crate::event_streaming::domain::runtime_store::RuntimeStore>,
    /// Operator state backend used by the checkpoint subsystem
    /// (Bloque C). Defaults to
    /// [`crate::event_streaming::domain::engine::state_store::InMemoryStateBackend`].
    pub state_backend: Arc<dyn crate::event_streaming::domain::engine::state_store::StateBackend>,
    ...
    pub flink_config: std::sync::Arc<crate::event_streaming::runtime::flink::FlinkRuntimeConfig>,
