...
| Read/write DNS | `report-pg-rw.openfoundry.svc.cluster.local:5432`             |
| Read-only DNS  | `report-pg-ro.openfoundry.svc.cluster.local:5432`             |
| Database       | `report`                                                       |
| App secret     | `report-pg-app` (CNPG-managed; carries `username`, `password`, `uri`) |
| Backup secret  | `report-pg-backup` (S3 credentials for barman-cloud)          |
...
`services.report-service.envSecrets.DATABASE_URL`
