...
| Field          | Value                                                              |
|----------------|--------------------------------------------------------------------|
| Manifest       | [`infra/k8s/platform/manifests/cnpg/clusters/nexus-pg.yaml`](../../../infra/k8s/platform/manifests/cnpg/clusters/nexus-pg.yaml) |
| `kind`         | `Cluster` (`postgresql.cnpg.io/v1`, CloudNativePG / cloudnative-pg)|
| Cluster name   | `nexus-pg`                                                   |
| Namespace      | `openfoundry`                                                      |
| Read/write DNS | `nexus-pg-rw.openfoundry.svc.cluster.local:5432`             |
| Read-only DNS  | `nexus-pg-ro.openfoundry.svc.cluster.local:5432`             |
| Database       | `nexus`                                                       |
| App secret     | `nexus-pg-app` (CNPG-managed; carries `username`, `password`, `uri`) |
| Backup secret  | `nexus-pg-backup` (S3 credentials for barman-cloud)          |
...
`services.nexus-service.envSecrets.DATABASE_URL`
