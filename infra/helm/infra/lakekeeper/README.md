...
[`infra/k8s/rook/bucket.yaml`](../rook/bucket.yaml)).
...
1. **CloudNativePG operator** — installed cluster-wide (see
   [`infra/k8s/helm/open-foundry/charts/iceberg-catalog/templates/cnpg-cluster.yaml`](../helm/open-foundry/charts/iceberg-catalog/templates/cnpg-cluster.yaml)
...
3. **Rook-Ceph RGW** with the `openfoundry-iceberg` bucket — see
   [`infra/k8s/rook/README.md`](../rook/README.md). The Service
...
4. **`identity-federation-service`** reachable at the OIDC issuer URL
   ...
   service is deployed by the parent
   [`infra/k8s/helm/open-foundry`](../helm/open-foundry) chart
...
kubectl apply -f infra/k8s/lakekeeper/namespace.yaml
...
helm upgrade --install lakekeeper lakekeeper/lakekeeper \
    ...
    --values infra/k8s/lakekeeper/values.yaml \
    --wait
...
* [`infra/k8s/helm/open-foundry/values.yaml`](../helm/open-foundry/values.yaml)
