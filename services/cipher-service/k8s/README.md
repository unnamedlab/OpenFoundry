...
| Field          | Value                                                              |
|----------------|--------------------------------------------------------------------|
| Manifest       | [`infra/k8s/platform/manifests/cnpg/clusters/cipher-pg.yaml`](../../../infra/k8s/platform/manifests/cnpg/clusters/cipher-pg.yaml) |
| `kind`         | `Cluster` (`postgresql.cnpg.io/v1`, CloudNativePG / cloudnative-pg)|
| Cluster name   | `cipher-pg`                                                   |
| Namespace      | `openfoundry`                                                      |
| Read/write DNS | `cipher-pg-rw.openfoundry.svc.cluster.local:5432`             |
| Read-only DNS  | `cipher-pg-ro.openfoundry.svc.cluster.local:5432`             |
| Database       | `cipher`                                                       |
| App secret     | `cipher-pg-app` (CNPG-managed; carries `username`, `password`, `uri`) |
| Backup secret  | `cipher-pg-backup` (S3 credentials for barman-cloud)          |
...
`services.cipher-service.envSecrets.DATABASE_URL`
