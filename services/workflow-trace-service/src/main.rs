...
#[tokio::main]
async fn main() {
    observability::init_tracing("workflow-trace-service");
    ...
    let public = Router::new().route(
        "/health",
        get(|| async { axum::Json(HealthStatus::ok("workflow-trace-service")) }),
    );
    ...
    let protected = Router::new()
        ...
        .route(
            "/api/v1/workflow-traces/runs/{id}",
            get(handlers::get_item),
        )
        .route(
            "/api/v1/workflow-traces/runs/{id}/secondary",
    ...
    tracing::info!("starting workflow-trace-service on {addr}");
