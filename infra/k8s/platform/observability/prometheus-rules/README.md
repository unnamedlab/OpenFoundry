...
|---|---|---|
| `kafka.yaml`      | Strimzi-managed Apache Kafka     | `platform/manifests/strimzi/`        |
| `vespa.yaml`      | Vespa.ai search                  | `platform/charts/vespa/`, `platform/packages/vespa-app/` |
| `lakekeeper.yaml` | Lakekeeper (Iceberg REST catalog)| `platform/manifests/lakekeeper/`     |
| `cnpg.yaml`       | CloudNativePG                    | `platform/manifests/cnpg/`           |
| `flink.yaml`      | Apache Flink + maintenance jobs  | `platform/manifests/flink/`, `platform/manifests/flink/maintenance/` |
...
promtool test rules infra/k8s/platform/observability/prometheus-rules/tests/*.yaml
```
...
OpenFoundry currently keeps monitoring rules outside the app Helm
charts. There is no `monitoring` platform release yet; when one is
added, these files should be templated into it as `PrometheusRule`
resources via the wrapper shown above. Until then, deploy them directly
as raw manifests:

```bash
# Manual deployment (one command per file):
for f in infra/k8s/platform/observability/prometheus-rules/*.yaml; do
