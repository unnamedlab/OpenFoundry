...
#[tokio::main]
async fn main() {
    observability::init_tracing("analytical-logic-service");
    ...
    let public = Router::new().route(
        "/health",
        get(|| async { axum::Json(HealthStatus::ok("analytical-logic-service")) }),
    );
    ...
    let protected = Router::new()
        ...
        .route(
            "/api/v1/analytical-logic/expressions/{id}",
            get(handlers::get_item),
        )
        .route(
            "/api/v1/analytical-logic/expressions/{id}/secondary",
    ...
    tracing::info!("starting analytical-logic-service on {addr}");
