...
pub async fn create_stream(
    ...
    // for legacy rows.
    let stream_rid = crate::event_streaming::models::stream_view::stream_rid_for(stream_id);
    let view_rid = crate::event_streaming::models::stream_view::view_rid_for(stream_id);
...
pub async fn update_stream(
    ...
    let new_kind = payload.kind.unwrap_or_else(|| {
        crate::event_streaming::models::stream_view::StreamKind::from_str(&existing.kind)
            .unwrap_or_default()
    });
    let mut tx = state.db.begin().await.map_err(|cause| db_error(&cause))?;
    sqlx::query(
...
pub async fn get_stream_metrics(
    ...
    let events = state
        ...
        .await
        .map_err(|cause| crate::event_streaming::handlers::internal_error(cause.to_string()))?;
...
mod tests {
    ...
    use crate::event_streaming::models::stream::{StreamField, StreamSchema};
