...
| Field          | Value                                                              |
|----------------|--------------------------------------------------------------------|
| Manifest       | [`infra/k8s/platform/manifests/cnpg/clusters/monitoring-rules-pg.yaml`](../../../infra/k8s/platform/manifests/cnpg/clusters/monitoring-rules-pg.yaml) |
| `kind`         | `Cluster` (`postgresql.cnpg.io/v1`, CloudNativePG / cloudnative-pg)|
| Cluster name   | `monitoring-rules-pg`                                                   |
| Namespace      | `openfoundry`                                                      |
| Read/write DNS | `monitoring-rules-pg-rw.openfoundry.svc.cluster.local:5432`             |
| Read-only DNS  | `monitoring-rules-pg-ro.openfoundry.svc.cluster.local:5432`             |
| Database       | `monitoring_rules`                                                       |
| App secret     | `monitoring-rules-pg-app` (CNPG-managed; carries `username`, `password`, `uri`) |
| Backup secret  | `monitoring-rules-pg-backup` (S3 credentials for barman-cloud)          |
...
`services.monitoring-rules-service.envSecrets.DATABASE_URL`
