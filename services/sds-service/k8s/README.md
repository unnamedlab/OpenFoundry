...
| Field          | Value                                                              |
|----------------|--------------------------------------------------------------------|
| Manifest       | [`infra/k8s/platform/manifests/cnpg/clusters/sds-pg.yaml`](../../../infra/k8s/platform/manifests/cnpg/clusters/sds-pg.yaml) |
| `kind`         | `Cluster` (`postgresql.cnpg.io/v1`, CloudNativePG / cloudnative-pg)|
| Cluster name   | `sds-pg`                                                   |
| Namespace      | `openfoundry`                                                      |
| Read/write DNS | `sds-pg-rw.openfoundry.svc.cluster.local:5432`             |
| Read-only DNS  | `sds-pg-ro.openfoundry.svc.cluster.local:5432`             |
| Database       | `sds`                                                       |
| App secret     | `sds-pg-app` (CNPG-managed; carries `username`, `password`, `uri`) |
| Backup secret  | `sds-pg-backup` (S3 credentials for barman-cloud)          |
...
`services.sds-service.envSecrets.DATABASE_URL`
