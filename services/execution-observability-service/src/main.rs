...
#[tokio::main]
async fn main() {
    observability::init_tracing("execution-observability-service");
    ...
    let public = Router::new().route(
        "/health",
        get(|| async { axum::Json(HealthStatus::ok("execution-observability-service")) }),
    );
    ...
    let protected = Router::new()
        ...
        .route(
            "/api/v1/execution-observability/runs/{id}",
            get(handlers::get_item),
        )
        .route(
            "/api/v1/execution-observability/runs/{id}/secondary",
    ...
    tracing::info!("starting execution-observability-service on {addr}");
