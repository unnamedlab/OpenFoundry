...
context is the **ingestion-replication-service** Postgres schema (migrations under
...
| Field          | Value                                                              |
|----------------|--------------------------------------------------------------------|
| Manifest       | [`infra/k8s/cnpg/clusters/ingestion-replication-pg.yaml`](../../../infra/k8s/cnpg/clusters/ingestion-replication-pg.yaml) |
| `kind`         | `Cluster` (`postgresql.cnpg.io/v1`, CloudNativePG / cloudnative-pg)|
| Cluster name   | `ingestion-replication-pg`                                                   |
| Namespace      | `openfoundry`                                                      |
| Read/write DNS | `ingestion-replication-pg-rw.openfoundry.svc.cluster.local:5432`             |
| Read-only DNS  | `ingestion-replication-pg-ro.openfoundry.svc.cluster.local:5432`             |
| Database       | `ingestion_replication`                                                       |
| App secret     | `ingestion-replication-pg-app` (CNPG-managed; carries `username`, `password`, `uri`) |
| Backup secret  | `ingestion-replication-pg-backup` (S3 credentials for barman-cloud)          |
...
`services.ingestion-replication-service.envSecrets.DATABASE_URL`
