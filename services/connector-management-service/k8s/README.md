...
context is the **connector-management-service** Postgres schema (migrations under
...
| Field          | Value                                                              |
|----------------|--------------------------------------------------------------------|
| Manifest       | [`infra/k8s/cnpg/clusters/connector-management-pg.yaml`](../../../infra/k8s/cnpg/clusters/connector-management-pg.yaml) |
| `kind`         | `Cluster` (`postgresql.cnpg.io/v1`, CloudNativePG / cloudnative-pg)|
| Cluster name   | `connector-management-pg`                                                   |
| Namespace      | `openfoundry`                                                      |
| Read/write DNS | `connector-management-pg-rw.openfoundry.svc.cluster.local:5432`             |
| Read-only DNS  | `connector-management-pg-ro.openfoundry.svc.cluster.local:5432`             |
| Database       | `connector_management`                                                       |
| App secret     | `connector-management-pg-app` (CNPG-managed; carries `username`, `password`, `uri`) |
| Backup secret  | `connector-management-pg-backup` (S3 credentials for barman-cloud)          |
...
`services.connector-management-service.envSecrets.DATABASE_URL`
