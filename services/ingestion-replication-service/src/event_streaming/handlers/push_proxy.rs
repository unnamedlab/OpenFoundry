...
async fn load_stream_row_by_rid(
    ...
    let Some(uuid_str) =
        stream_rid.strip_prefix(crate::event_streaming::models::stream_view::STREAM_RID_PREFIX)
    else {
        return Ok(None);
    };
...
pub async fn push_records(
    ...
    for (value, event_time, key) in records.iter() {
        let payload_bytes = serde_json::to_vec(value).map_err(|cause| {
            ...
            crate::event_streaming::handlers::internal_error("payload re-serialise failed")
        })?;
        let key_ref = key
            .as_deref()
