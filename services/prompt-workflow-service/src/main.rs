...
#[tokio::main]
async fn main() {
    observability::init_tracing("prompt-workflow-service");
    ...
    let public = Router::new().route(
        "/health",
        get(|| async { axum::Json(HealthStatus::ok("prompt-workflow-service")) }),
    );
    ...
    tracing::info!("starting prompt-workflow-service on {addr}");
