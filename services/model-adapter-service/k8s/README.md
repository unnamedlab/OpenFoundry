...
| Field          | Value                                                              |
|----------------|--------------------------------------------------------------------|
| Manifest       | [`infra/k8s/platform/manifests/cnpg/clusters/model-adapter-pg.yaml`](../../../infra/k8s/platform/manifests/cnpg/clusters/model-adapter-pg.yaml) |
| `kind`         | `Cluster` (`postgresql.cnpg.io/v1`, CloudNativePG / cloudnative-pg)|
| Cluster name   | `model-adapter-pg`                                                   |
| Namespace      | `openfoundry`                                                      |
| Read/write DNS | `model-adapter-pg-rw.openfoundry.svc.cluster.local:5432`             |
| Read-only DNS  | `model-adapter-pg-ro.openfoundry.svc.cluster.local:5432`             |
| Database       | `model_adapter`                                                       |
| App secret     | `model-adapter-pg-app` (CNPG-managed; carries `username`, `password`, `uri`) |
| Backup secret  | `model-adapter-pg-backup` (S3 credentials for barman-cloud)          |
...
`services.model-adapter-service.envSecrets.DATABASE_URL`
