# `automation-operations-service` — Kubernetes binding

This service runs as part of the umbrella Helm chart at
`infra/k8s/helm/open-foundry/`. The only state owned by this bounded
context is the **automation-operations-service** Postgres schema (migrations under
...
| Field          | Value                                                              |
|----------------|--------------------------------------------------------------------|
| Manifest       | [`infra/k8s/platform/manifests/cnpg/clusters/automation-operations-pg.yaml`](../../../infra/k8s/platform/manifests/cnpg/clusters/automation-operations-pg.yaml) |
| `kind`         | `Cluster` (`postgresql.cnpg.io/v1`, CloudNativePG / cloudnative-pg)|
| Cluster name   | `automation-operations-pg`                                                   |
| Namespace      | `openfoundry`                                                      |
| Read/write DNS | `automation-operations-pg-rw.openfoundry.svc.cluster.local:5432`             |
| Read-only DNS  | `automation-operations-pg-ro.openfoundry.svc.cluster.local:5432`             |
| Database       | `automation_operations`                                                       |
| App secret     | `automation-operations-pg-app` (CNPG-managed; carries `username`, `password`, `uri`) |
| Backup secret  | `automation-operations-pg-backup` (S3 credentials for barman-cloud)          |
...
`services.automation-operations-service.envSecrets.DATABASE_URL`
